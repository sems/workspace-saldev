        ...
        if(!isset($_SESSION['logged_in']) || ($_SESSION['logged_in'] == false) || ($_SESSION['gebruiker_rank'] == NULL) || ($_SESSION['gebruiker_rank'] == 0)) {
            ...
            if(isSet ($_POST['title']) && ($_POST['portfolio_content'])) {
                try {
                    ...
                    $postAuthor = $_SESSION['gebruiker'];
                    // If there is a file in the input then run the following code:
                    if($uploadedFile["error"] === 0 && $uploadedFile["size"] > 0) {
                        //The query for the blogpost when there *is* a image present.
                        $queryIMG = "INSERT INTO `portfolio`(`title`, `post`, `portfolio_image`, `post_desc`, `post_author`) VALUES (:title, :post, :portf_img, :postdesc, :author)";
                        ...
                        $dbinsertIMG->bindParam(':author', $postAuthor, PDO::PARAM_STR);
                        //print_r($dbinsertIMG);
                        $dbinsertIMG->execute();
                        ...
                        $query = "INSERT INTO `portfolio`(`title`, `post`, `post_desc`, `post_author`) VALUES (:title, :post, :postdesc, :author )";
                        ...
                        $dbinsert->bindParam(':author', $postAuthor, PDO::PARAM_STR);
                        //print_r($dbinsert);
                        $dbinsert->execute();
